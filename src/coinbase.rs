...
/// Bitcoin Core builds `CScript() << nHeight` using push_int64, which emits OP_1..OP_16
/// (0x51..0x60) for heights 1–16 — NOT a length-prefixed CScriptNum push.
fn push_bip34_height(buf: &mut Vec<u8>, height: u32) {
    match height {
        0 => buf.push(0x00), // OP_0 — genesis only, never mined
        1..=16 => buf.push(0x50 + height as u8), // OP_1 (0x51) .. OP_16 (0x60)
        _ => {
            // CScriptNum minimal encoding for height >= 17
            let mut n = height;
            ...
            // Height is always positive; if high bit of last byte is set, append 0x00 sign byte.
